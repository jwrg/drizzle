...
class Platelet:
    ...
    pump_zone = None if current_app.config['PUMP_ZONE'] == None else Zone('Pump', current_app.config['PUMP_ZONE'][0], current_app.config['PUMP_ZONE'][1])
    ...
    def pumpOn(time: int):
        if Platelet.pump_zone is not None:
            ...
            Platelet.logger.info(' '.join([
            'Pump was turned on for', 
            str(time),
            'minute.' if time == 1 else 'minutes.'
            ]))
        else:
            Platelet.logger.debug('Call to pumpOn() but pump NOT turned on; no pump zone set')
    def pumpOff():
        if Platelet.pump_zone is not None:
