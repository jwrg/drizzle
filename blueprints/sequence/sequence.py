...
@sequence.route('/')
def list():
    actions = { 'run': 'sequence.runSequence',
...
def runSequence(id):
    ...
    return redirect(url_for('sequence.list'))
...
def newSequence():
    ...
    if request.method == 'POST':
        return redirect(url_for('sequence.editSequence', id = id), code = 307)
...
def editSequence(id):
    ...
    return render_template('edit.html',\
            ...
            fields = ['name', 'description', 'sequence'],\
            constrain = {'minutes': current_app.config['MAX_TIME'],\
                'zone':[ x for x in range(1, current_app.config['NUM_ZONES'] + 1)]},\
            num_zones = current_app.config['NUM_ZONES'])
