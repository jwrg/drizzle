...
def zoneSelect():
    ...
    actions = [
        (
            str(x),
            "zone.disable" if x in state else "zone.timeSelect",
            {"zone": x},
            x in state,
        )
...
def timeSelect(zone):
    actions = [
        (str(x), "zone.enable", {"zone": zone, "time": x}, False)
        for x in current_app.config["TIMES"]
    ]
    actions.append(("Cancel", "index", {}, False))
    prompt = " ".join(
        ["Activate zone", str(zone) if zone != 0 else "pump", "for how many minutes?"]
    )
    return render_template(
        "keypad.html", confirm=True, subject="time", prompt=prompt, actions=actions
    )
...
def disable(zone):
    if zone == 0:
        ...
        flash(" ".join(["Zone", str(zone), "was turned off."]), "success")
